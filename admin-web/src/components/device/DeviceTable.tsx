...
function DeviceTable({ devices, isLoading }: Props) {
            ...
            <tr key={device.deviceId} className="hover:bg-[#f2f3f8]">
              ...
              <td className="p-[1rem] font-semibold">
                {device.trusted ? (
                  ...
                  <span className="px-2 py-1 text-danger">Chưa xác thực</span>
                )}
              </td>

              <td className="p-[1rem]">
                {device.registeredDate &&
                  parseSafeDate(device.registeredDate)?.toLocaleString("vi-VN")}
              </td>

              <td className="p-[1rem]">
                {device.lastUsedDate &&
                  parseSafeDate(device.lastUsedDate)?.toLocaleString("vi-VN")}
              </td>
